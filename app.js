...
function loadCaseStudies(baseDir) {
  ...
  for (const study of studyDirs) {
    ...
    let tables = [];
    console.log(`Checking for appendix in: ${study.name}`);
    console.log(`Appendix path: ${appendixPath}`);
    console.log(`Appendix exists: ${fs.existsSync(appendixPath)}`);
    
    if (fs.existsSync(appendixPath)) {
      const appendixFiles = fs.readdirSync(appendixPath);
      console.log(`Appendix files in ${study.name}:`, appendixFiles);
      const tablePatterns = ['f1', 'precision', 'recall', 'kappa', 'presnost'];
      const suffixPatterns = ['_tabulka', '_graf'];
      ...
      console.log(`Filtered tables for ${study.name}:`, tables);
    } else {
      console.log(`No appendix directory found for ${study.name}`);
    }

    // Najdi metody klasifikace
    const allDirs = fs.readdirSync(studyPath, { withFileTypes: true })
      .filter(d => d.isDirectory());
    console.log(`All directories in ${study.name}:`, allDirs.map(d => d.name));
    
    const methods = allDirs
      .filter(d => !d.name.endsWith('.fld') && d.name !== 'appendix')
      .map(method => {
        const methodPath = path.join(studyPath, method.name);
        console.log(`Checking method directory: ${methodPath}`);
        const allFiles = fs.readdirSync(methodPath);
        console.log(`All files in ${method.name}:`, allFiles);
        ...
        console.log(`Filtered images for ${method.name}:`, images);
        return {
          name: method.name,
          images
        };
...
app.get("/", (req, res) => {
  ...
  console.log("Loaded case studies:", JSON.stringify(caseStudies, null, 2));
